...
export default function SocietiesPage() {
  ...
  const [editingSociety, setEditingSociety] =
    useState<Society | null>(null);

  const [societyForm, setSocietyForm] =
    useState<SocietyForm>(emptySocietyForm);

  const [savingSociety, setSavingSociety] =
    useState(false);
    ...
    try {
      ...
      if (showRefresh) {
        toast({
          ...
          description:
            'Latest society data has been loaded.',
        });
      }
    } catch (error: any) {
      console.error(
  ...
  const saveSociety = async (
    ...
    const payload = {
      name:
        societyForm.name.trim(),
      society_code:
        societyForm.society_code
          .trim()
          .toUpperCase(),
      address:
        societyForm.address.trim(),
      city:
        societyForm.city.trim(),
      state:
        societyForm.state.trim(),
      contact_number:
        societyForm.contact_number.trim(),
    };
    ...
    if (
      ...
      toast({
        ...
        description:
          'Please fill all society details.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      const response =
        society.is_active
          ...
          : await api.put(
              `/societies/${society._id}/activate`
            );
  ...
  // ============================================================
  // OPEN SOCIETY DETAILS / MANAGER
  // ============================================================

  const openSocietyDetails = (
    society: SocietyWithData
  ) => {
    router.push(
      `/admin/societies/${society._id}`
    );
  };
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-700 mt-1">
              ...
              Each society has its own manager,
          ...
          >
            ...
            <div className="rounded-lg bg-slate-50 border border-slate-200 p-3">
              ...
              <p className="text-xs text-slate-600">
                Society Code is used by residents,
                admins and managers during
                login/registration.
              </p>
